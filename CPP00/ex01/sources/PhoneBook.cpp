...
/*   Updated: 2024/02/15 20:46:15 by achabrer         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
{
	const std::string	prompts[] = {"Enter a name", "Enter a last name", "Enter a nickname",
	...
	std::cout << std::endl << attributes[0] << " added to the book, press ENTER to continue" << std::endl;
	std::cin.ignore(std::numeric_limits<std::streamsize>::max(), '\n');
}
...
void	PhoneBook::search(void)
{
	...
	this->_display();
	std::cout << "Enter the index of the contact you want to look up" << std::endl;	
	while (42)
	{
		std::cin >> id;
		if (std::cin.fail())
