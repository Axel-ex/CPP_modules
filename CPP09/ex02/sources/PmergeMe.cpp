...
void	PmergeMe :: compute( void )
{
	...
	std::cout << "Time to process a range of " << _input1.size() << " with std::vector "
	<< " " << (t_vec).count() <<  " us" << std::endl;
	std::cout << "Time to process a range of " << _input2.size() << " with std::deque "
	<< " " << (t_deq).count() <<  " us" << std::endl;
}

std::chrono::high_resolution_clock::duration	PmergeMe :: timeExecution( std::string t_container )
{
...
{
	size_t	l = 0; size_t	r = 0; size_t	o = 0;
