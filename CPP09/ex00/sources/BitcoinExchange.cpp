...
void	BitcoinExchange :: loadDataBase( void )
{
	...
	while (std::getline(ifs, row, '\n'))
	{
		...
		curr.second = std::strtof(row.substr(row.find(',') + 1).c_str(), nullptr);
		_data.insert(curr);
	}
	ifs.close();
}
...
void BitcoinExchange :: compute( void )
{
	...
	ifs.open(_file_name);
	if (ifs.fail())
		throw std::ifstream::failure("Error: could not open the file " + _file_name);
 ...
 * 
 * if the date is not properly formatted, converting it to a time_t and then back
 * to a string will yield a different string (mktime interprets the date if wrong
 * input is provided)
 * 
 * @param date 
 */
